...
function buildRecord(mail, { events, sends, personReplies, index, coldAfterDays, now, full }) {
  ...
  const base = {
    ...
    // İnsan-yazımı (imported) mailleri AI-üretiminden ayır.
    source: mail.source ?? "generated",
    authored_by: mail.authored_by ?? null,
...
export async function mailAnalytics(workspace, { now = () => new Date() } = {}) {
  ...
  return {
    ...
    by_source: breakdown(records, (r) => r.source === "imported" ? "insan (import)" : "AI"),
